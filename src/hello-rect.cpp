...
int main()
{
   ...
   // drawing rectangle so we will setup EBO for indices.
    ...
    // binding ebo to respective buffer and store the indices data in gpu using ebo
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,EBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,sizeof(indices),indices,GL_STATIC_DRAW);
    // now we need to say gldrawelements not gldrawarrays in render loop
    // to draw using indices from EBO currently bound, and get vertex data from currently bound 
    // VBO i.e. VBO which has GL_ARRAY_BUFFER target
    // The glDrawElements function takes its indices from the EBO currently bound to the GL_ELEMENT_ARRAY_BUFFER target, which is cumbersome as each time we 
    // need to bind EBO when drawing an object, better is that VAO stores EBO too.
    

    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void *)0);
    glEnableVertexAttribArray(0);

// unbinding VBO as its job to be used by vertex attribute is done by above line by vertexattribpointer
    glBindBuffer(GL_ARRAY_BUFFER,0);

    // glBindVertexArray(0);
// uncomment below line for seeing line part only.
